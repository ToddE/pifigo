...
// Start begins the boot management timer process.
...
// revertToLastGoodConfig copies the file pointed to by the symlink.
func revertToLastGoodConfig() {
	if _, err := os.Lstat(lastGoodSymlink); os.IsNotExist(err) {
		log.Println("No last-good WiFi configuration symlink found. Remaining in hotspot mode.")
		return
	}
	stopCmd := ExecCommand("sh", "-c", "systemctl stop hostapd dnsmasq pifigo")
	if output, err := stopCmd.CombinedOutput(); err != nil {
		log.Printf("ERROR: Boot manager failed to stop hotspot services: %v\nOutput: %s", err, string(output))
	}
	copyCmd := ExecCommand("cp", lastGoodSymlink, activeClientConfig)
	if err := copyCmd.Run(); err != nil {
		log.Printf("ERROR: Boot manager failed to copy last-good config: %v", err)
		return
	}
	applyCmd := ExecCommand("netplan", "apply")
	if err := applyCmd.Run(); err != nil {
		log.Printf("ERROR: Boot manager failed to apply last-good WiFi config: %v", err)
	}
}

// ForceHotspotMode reverts the system to its default hotspot state.
